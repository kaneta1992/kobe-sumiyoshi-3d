...
const SKY_SCRIPT = [
    ...
    { alt: -0.04, sun: [0.86, 0.42, 0.3], sunI: 0.62, zenith: [0.06, 0.08, 0.29], horizon: [0.27, 0.27, 0.45], haze: [0.72, 0.28, 0.14], amb: 0.44 },
    // 夜の底。ここを下げすぎると「暗闇で何も見えない」になるので、月光（sunI）と
    // 空からの環境光（amb）は**影の中でも地形と建物のシルエットが読める**線で止める。
    // 色を青に寄せたまま明るさだけ持つので、夜の雰囲気は落とさない
    { alt: -0.14, sun: [0.55, 0.66, 1], sunI: 0.46, zenith: [0.035, 0.05, 0.13], horizon: [0.12, 0.14, 0.23], haze: [0.14, 0.16, 0.25], amb: 0.32 },
] as const;
